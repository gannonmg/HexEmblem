...
> Source of truth: [`.docs/diagrams/.dependency-graph.mmd`](.docs/diagrams/.dependency-graph.mmd)
> (ELK layout). It's hidden because GitHub can't render ELK inline; the committed
> SVG is regenerated by a pre-commit hook whenever the `.mmd` changes (see the
> repo root README's *Project setup*). **Edit the `.mmd`, not the SVG.**
